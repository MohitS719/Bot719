    ...
    if chat.type == chat.PRIVATE:
        if len(args) >= 1:
            if args[0] in ("yes", "on"):
                ...
                msg.reply_text("Turned on reporting! ab koi chutiyapanti hogi admins ko pata chal jaega.")

            elif args[0] in ("no", "off"):
                sql.set_user_setting(chat.id, False)
                msg.reply_text("Turned off reporting! bc fir chudap hote rahenge group me.")
        else:
...
def report(bot: Bot, update: Update) -> str:
    ...
    if chat and message.reply_to_message and sql.chat_should_report(chat.id):
        ...
        if chat.username and chat.type == Chat.SUPERGROUP:
            ...
            msg = "{} Lawdey admins kaha gye? \"{}\"!".format(mention_html(user.id, user.first_name),
