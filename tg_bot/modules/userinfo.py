...
def about_me(bot: Bot, update: Update, args: List[str]):
    ...
    if info:
        ...
        update.effective_message.reply_text(username + "Information about this Londa isnt available !")
    else:
...
def set_about_me(bot: Bot, update: Update):
    ...
    if len(info) == 2:
        if len(info[1]) < MAX_MESSAGE_LENGTH // 4:
            ...
            message.reply_text(
                " About You{} To be explained in words ".format(MAX_MESSAGE_LENGTH // 4, len(info[1])))
...
def set_about_bio(bot: Bot, update: Update):
    ...
    if message.reply_to_message:
        ...
        if user_id == message.from_user.id:
            ...
            message.reply_text(" Only SUDO USERS (owner se chote owner)can change my information.")
            return
