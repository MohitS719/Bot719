...
def add_blacklist(bot: Bot, update: Update):
    ...
    if len(words) > 1:
        ...
        msg.reply_text("konsa word nikalna h blacklist se?")
...
def unblacklist(bot: Bot, update: Update):
    ...
    if len(words) > 1:
        ...
        if len(to_unblacklist) == 1:
            ...
            msg.reply_text(
                "Tried to find but nhi mile to remove bhi nhi hue.".format(
    ...
    for trigger in chat_filters:
        ...
        if re.search(pattern, to_match, flags=re.IGNORECASE):
            try:
                ...
                if excp.message == "Message to be deleted not found":
                    pass
                else:
...
def __chat_settings__(chat_id, user_id):
    ...
    return "There are {} blacklisted words(galiya aur baki ke chudap).".format(blacklisted)
...
__help__ = """
Blacklists are used to stop certain words or sentence to be sent in the group. Any time blacklist wala word koi likhega to msg delete ho jaega gand marke, \
. !

*NOTE:* blacklists admins ke msg delete nhi karega.
