...
def lock(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if can_delete(chat, bot.id):
        ...
        message.reply_text("Admin nhi h mere pass pehle admin banao muje.")
                ...
                for new_mem in new_members:
                    if new_mem.is_bot:
                        if not is_bot_admin(chat, bot.id):
                            message.reply_text("yaha ek bot join hua h aur muje bola gaya h ki bot ko pehli fursat me nikalu "
                                               "but I'm not admin!")
                            return

                        chat.kick_member(new_mem.id)
                        message.reply_text("Madrchod sirf admin bot ko la sakta h group me to pehli fursat me nikl")
            else:
                try:
                    message.delete()
