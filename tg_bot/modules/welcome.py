...
def send(update, message, keyboard, backup_message):
    try:
        ...
        msg = update.effective_message.reply_text(markdown_parser(backup_message +
                                                                  ...
                                                                  "curly brackets ye wale nhi chalte bc Please update"),
                                                  parse_mode=ParseMode.MARKDOWN)
...
def del_joined(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if not args:
        ...
        if del_pref:
            ...
            update.effective_message.reply_text("I'm currently not deleting old joined messages!NOT RECOMENDED")
        return ""
    ...
    if args[0].lower() in ("on", "yes"):
        ...
        update.effective_message.reply_text("I understand 'on/yes' or 'off/no' only!Jyada bakchodi kari to gand me sariya daldega mera Bot")
        return ""
    ...
    if should_welc:
        ...
        for new_mem in new_members:
            ...
            if new_mem.id == OWNER_ID:
                update.effective_message.reply_text("Londaa is in the houseeee, let's get this fuckin party started! Where are the Hoes?")
                continue
    ...
    if should_goodbye:
        ...
        if left_mem:
            ...
            if left_mem.id == OWNER_ID:
                update.effective_message.reply_text("Londaa kyu chala gya?BC chutiye log roko use")
                return
...
def welcome(bot: Bot, update: Update, args: List[str]):
    ...
    if len(args) == 0 or args[0].lower() == "noformat":
        ...
        if args[0].lower() in ("on", "yes"):
            ...
            update.effective_message.reply_text("I'll be Ghantaa polite!")

        elif args[0].lower() in ("off", "no"):
            sql.set_welc_preference(str(chat.id), False)
            update.effective_message.reply_text("I'm sulking,Kisi ko hello nhi bolunga bc.")

        else:
            # idek what you're writing, say yes or no
            update.effective_message.reply_text("I understand 'on/yes' or 'off/no' only!Jyada bakchodi kari to gand me sariya daldunga")
...
def goodbye(bot: Bot, update: Update, args: List[str]):
    ...
    if len(args) == 0 or args[0] == "noformat":
        ...
        if args[0].lower() in ("on", "yes"):
            ...
            update.effective_message.reply_text("Bc vaps aya to gand mardunga!")
...
WELC_HELP_TXT = "Your group's welcome/goodbye messages can be setted in many londey ways. If you want the messages" \
                " to be Different for every person , like the lawde ka user name aur sab chutiyappa, you can use *these* variables:\n" \
                " - `{{first}}`:  user's *first* name\n" \
                " - `{{last}}`: user's *last* name. *first name* ban jaega agar the last name isnt there. " \
                "last name.\n" \
                " - `{{fullname}}`: user's *full* name.*first name*ban jaega agar the last name isnt there " \
                "last name.\n" \
                " - `{{username}}`: user's *username*. Simple *mention* ho jaega agar username isnt there. " \
                "first name if has no username.\n" \
                " - `{{mention}}`: this simply *mentions* a user - first name se tag hoga.\n" \
                " - `{{id}}`:  user's *id* given by servers if servers of telegram willnt be there this shit will not work\n" \
                " - `{{count}}`:  user's *member number*.\n" \
                " - `{{chatname}}`: user's *current chat name*.\n" \
                "\nEach variable (ye uper wale) `{{}}` to be replaced. Nhi to work nhi karega aur error dega\n" \
                "Welcome messages also support custome style(not totally custome tg se provided hi use ho sakte h), so you can make any word or text bold/italic/code/links/lawda. " \
                "Buttons are also supported, so you can make your welcomes look awesome with some nice intro " \
                "buttons.\n" \
                "To create a button linking to your rules(a simple command bhi use ho sakta ye sab karna important nhi h ), use this: `[Rules](buttonurl://t.me/{}?start=group_id)`. " \
                "Simply replace `group_id` with your group's id, which can be obtained via /id, and you're good to " \
                "go. Note that group ki ids me aisa sign hota h `-` sign; this is required, so please don't " \
                "remove it.\n" \
                "If you're feeling fun, you can even set images/gifs/videos/voice messages as the welcome message by " \
                "replying to the desired media, and calling /setwelcome.(few of these features are taken from Official Welcomebot *copyright*)".format(dispatcher.bot.username)
